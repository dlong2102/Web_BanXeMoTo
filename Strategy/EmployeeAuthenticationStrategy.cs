...
{
    public class EmployeeAuthenticationStrategy : IAuthenticationStrategy
    {
        public async Task<bool> AuthenticateAsync(string email, string password, HttpContext httpContext, QLMoToContext database)
        {
            var employee = await database.NhanViens.Where(x => x.Email == email && x.Pass == password).FirstOrDefaultAsync();
